...
// Using appsflyer s2s go package demo and adding functionality on top of it
...
const (
	...
	appsflyerFlightBookingDateFormat = "2006-01-02"
...
func (t Tracker) Send(evt *Event) error {
	...
	fmt.Println("====appsFlyerURL===: ", appsFlyerURL)
	...
	fmt.Println("===devkey===", string(appInfo.DevKey))
...
type Event struct {
	body
	platform deviceOS
	values   map[EventParam]interface{}
}
...
func NewEvent(appsFlyerID string, platform deviceOS) *Event {
	return &Event{
		...
		platform: platform,
		values:   make(map[EventParam]interface{}),
	}
}
...
func (evt *Event) SetDateValueFlight(param EventParam, val time.Time) *Event {
	evt.values[param] = val.Format(appsflyerFlightBookingDateFormat)
	return evt
}
